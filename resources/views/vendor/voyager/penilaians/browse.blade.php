...
<div class="container-fluid">
    ...
    <div class="browse-filters-holder" data-url="{{ Request::url() }}">
        ...
        <span class="filter-selector">
            ...
            <select id="filter-selector-{{ $key }}" name="filter-selector[]"
                data-column="{{ $filter['filter_column'] }}" class="filter-select select2">
                <option value="">---</option>
                ...
                <option value="{{ $item['id'] }}" @if ($val && $item['id']==$val) selected @endif>
                    @if($item['level'] > 0) {{ str_repeat("--", $item['level']) }} @endif {{
                    ...
                    <div class="table-responsive">
                        <table id="dataTable" class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th class="@if(isset($row->details->browse_align)){{ $row->details->browse_align }}@endif"
                                        @if(isset($row->details->browse_width)) style="width:{{
                                        $row->details->browse_width }}"@endif>
                            ...
                            <tbody>
                                ...
                                <tr data-record-id="{{$data->getKey()}}" data-slug="{{$dataType->slug}}"
                                    ...
                                    <td class="@if(isset($row->details->browse_align)){{ $row->details->browse_align }}@endif"
                                        @if(isset($row->details->browse_font_size)) style="font-size:{{
                                        $row->details->browse_font_size }}"@endif>
                                        ...
                                        <img src="{{ $adv_image->getFullUrl() }}"
                                            style="height: {{ $row->details->browse_image_max_height?? '50px' }}; width:auto">
                                        @endif
                                        ...
                                        <a href="{{ $adv_file->getFullUrl() }}">
                                            <img class="file-type"
                                                src="{{ voyager_extension_asset('icons/files/'.explode('/', $adv_file->mime_type)[1].'.svg') }}"
                                                style="height: {{ $row->details->browse_image_max_height?? '20px' }}; width:auto">
                                        </a>
                                        @endif
                                        @endforeach
                                        @endif
                                        ...
                                        <input type="checkbox" data-id="{{ $data->id }}" name="{{ $row->field }}"
                                            @if($data->{$row->field}) checked @endif class="tiny-toggle"
                                        data-tt-type="dot" data-tt-size="tiny">
                                        @else
                                        @if($data->{$row->field})
                                        ...
                                        <div class="text-field-holder">
                                            ...
                                            <div class="browse-inline-editor">
                                                <input class="browse-inline-input" data-id="{{ $data->id }}"
                                                    @if($row->type == 'number') type="number" @else type="text" @endif
                                                name="{{$row->field}}" value="{{ $data->{$row->field} }}">
                                                <button class="text-inline-save" type="button"
                                                    title="@lang('voyager-extension::bread.inline_save')"><i
                                                        class="voyager-check"></i></button>
                                                <button class="text-inline-cancel" type="button"
                                                    title="@lang('voyager-extension::bread.inline_cancel')"><i
                                            ...
                                            <div class="browse-text-holder">
                                                ...
                                                <a
                                                    href="{{ route('voyager.'.$dataType->slug.'.'.$row->details->url, $data->{$data->getKeyName()}) }}">
                                                    @elseif(isset($row->details->route) &&
                                                    isset($row->details->route->name) &&
                                                    isset($row->details->route->param_field))
                                                    <a
                                                        href="{{ route($row->details->route->name, $data->{$row->details->route->param_field}) }}">
                                                        @endif
                                                    ...
                                                    @endif
                                                    @if(isset($row->details->browse_inline_editor))
                                                    <button class="text-inline-edit" type="button"
                                                        title="@lang('voyager-extension::bread.inline_edit')"><i
                                                            class="voyager-edit"></i></button>
                                                    @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="browse-group-fields">
                                            ...
                                            @endforeach
                                            @else
                                            <i class="voyager-dot"></i><i class="voyager-dot"></i><i
                                                class="voyager-dot"></i>
                                            @endif
                                            @if(property_exists($row->details, 'browse_inline_editor'))
                                            <button data-name="{{$row->field}}" class="group-inline-edit" type="button"
                                                title="@lang('voyager-extension::bread.inline_edit')"><i
                                                    class="voyager-edit"></i></button>
                                            @endif
                                        </div>
                                        @elseif($row->type == 'adv_json' || $row->type == 'adv_related' || $row->type ==
                                        'adv_page_layout' || $row->type == 'adv_inline_set')
